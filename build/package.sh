...
per_target() {
  ...
  sh build/script/get-source.sh | sed "s:^:.cache/tmp/$TARGET_TRIPLE/:" | sed s/\$/.o/ | xargs llvm-ar -c -r -s ".cache/tmp/$TARGET_TRIPLE/libmb_rt.a"
  (cd ".cache/tmp/$TARGET_TRIPLE" && zip -9 -r "../../../out/$TARGET_TRIPLE.zip" libmb_rt.a)
}
